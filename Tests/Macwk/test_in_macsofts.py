...
class TestInMacsofts(object):
    ...
    @allure.severity("NORMAL")  # allure.severity:标记测试用例级别
    ...
    def test_in_macsofts(self, env, case, http, expected):
        ...
        with allure.step("准备发起请求"):
            allure.attach("请求方式：", http['method'])
            allure.attach("请求地址：", env['host']['macwk'], http['path'])
            allure.attach("请求参数：", http['params'])
        with allure.step("装载数据,发起请求"):
            self.request = requests.request(http['method'], url=env['host']['macwk'] + http['path'],
